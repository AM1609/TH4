...
public class BoardManager : NetworkBehaviour
{
    ...
    private void OnClickCell(int r, int c)
    {
        ...
        if (NetworkManager.Singleton.IsHost && GameManger.Instance.currentTurn.Value == 0)
        {
            ...
            CheckResult(r, c);
            GameManger.Instance.currentTurn.Value = 1;
        }

        // If button is clicked by client, then change button sprite as O

        else if (!NetworkManager.Singleton.IsHost && GameManger.Instance.currentTurn.Value == 1)
        {
    ...
    private void ChangeSpriteServerRpc(int r, int c)
    {
        buttons[r, c].GetComponent<Image>().sprite = oSprite;
        buttons[r, c].interactable = false;
        GameManger.Instance.currentTurn.Value = 0;
    ...
    private void CheckResult(int r, int c)
    {
        if (IsWon(r, c))
        {
            GameManger.Instance.ShowMsg("won");
        }
        else
        {
            if (IsGameDraw())
            {
                GameManger.Instance.ShowMsg("draw");
            }
        }
    }
